...
public class FSMPlayer : FSMBase
{
    ...
    private void OnSkillAction()
    {
        ...
        else if (Input.GetKeyDown(KeyCode.E) && CanUseSkill(1))
        {
            SkillCommands[1].Execute(gameObject);
        }
        else if (Input.GetKeyDown(KeyCode.Q) && CanUseSkill(2))
        {
            SkillCommands[2].Execute(gameObject);
        }
        else if (Input.GetKeyDown(KeyCode.LeftShift) && CanUseSkill(3))
        {
    ...
    protected virtual IEnumerator DEAD()
    {
        ...
        else
        {
            ...
            Mana.currentMP = Mana.MP;
            Health.LifeCount = 3;
            UIManager.Instance.Teleport("Town");
            SetState(CH_STATE.IDLE);
        }
    }
}